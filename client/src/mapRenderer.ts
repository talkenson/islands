...
export class MapRenderer {
  ...
  private viewport: Viewport = { scale: 1, ox: 0, oy: 0, zoom: 1 };
  ...
  zoom(deltaY: number, minZoom: number, maxZoom: number, actor: Actor, chunks: Map<string, ChunkSnapshot>): void {
  ...
  draw(actor: Actor, chunks: Map<string, ChunkSnapshot>): void {
    ...
    this.viewport = { scale: tile, ox: centerX, oy: centerY, zoom: this.viewport.zoom };
  ...
  private drawChunk(
    ...
    for (let i = 0; i < CHUNK_SIZE * CHUNK_SIZE; i++) {
      ...
      if (x + tile < 0 || y + tile < 0 || x > this.canvas.width || y > this.canvas.height) {
        continue;
      }
  ...
  private cellColor(chunk: ChunkSnapshot, chunks: Map<string, ChunkSnapshot>, index: number, wx: number, wy: number): string {
    ...
    if (waterKind !== WATER_NONE) {
      if (waterKind === WATER_RIVER) {
        const baseColor = this.colorNoise.noise2D(wx * 0.2, wy * 0.2) > 0.55
    ...
    if ((coverFlags & COVER_FLAG_MOUNTAIN) !== 0) {
      const n = this.colorNoise.octaveNoise2D(wx * 0.13 + 11, wy * 0.13 + 19, 3, 0.5);
    ...
    if (soil === SOIL_SAND) {
      const n = this.colorNoise.octaveNoise2D(wx * 0.11 + 7, wy * 0.11 + 13, 3, 0.52);
  ...
  private biomeColors(biome: number): string[] {
    return this.renderConfig.palette.biomes[String(biome)]
  ...
  private hasWaterNeighbor(chunks: Map<string, ChunkSnapshot>, wx: number, wy: number): boolean {
    return this.waterKindAt(chunks, wx - 1, wy) !== WATER_NONE
      || this.waterKindAt(chunks, wx + 1, wy) !== WATER_NONE
      || this.waterKindAt(chunks, wx, wy - 1) !== WATER_NONE
      || this.waterKindAt(chunks, wx, wy + 1) !== WATER_NONE;
  }

  private waterKindAt(chunks: Map<string, ChunkSnapshot>, wx: number, wy: number): number {
