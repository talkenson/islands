...
interface EntityPatchPayload extends ActionResult {
  actor?: ActorWire;
}

interface ColorNoise {
...
(function () {
  ...
  function makeSeed(seed: string, salt: string): string {
    return `${seed}:${salt}`;
  }
  ...
  const state: State = {
    ...
    worldID: 1,
    chunks: new Map<string, ChunkSnapshot>(),
    lastEventID: 0,
    busy: false,
    streamAbort: null,
  ...
  const statusBox = element<HTMLElement>("status");
  const statusText = element<HTMLElement>("statusText");
  const emptyState = element<HTMLElement>("emptyState");
  const actorPosition = element<HTMLElement>("actorPosition");
  const chunkCount = element<HTMLElement>("chunkCount");
  const cellStock = element<HTMLElement>("cellStock");
  const eventLog = element<HTMLOListElement>("eventLog");

  const buttons = {
    up: element<HTMLButtonElement>("moveUp"),
    down: element<HTMLButtonElement>("moveDown"),
    left: element<HTMLButtonElement>("moveLeft"),
    right: element<HTMLButtonElement>("moveRight"),
    harvest: element<HTMLButtonElement>("harvest"),
  ...
  function normalizeActor(actor: ActorWire): Actor {
    return {
  ...
  function handleSSE(raw: string): void {
  ...
  function normalizeChunk(data: ChunkSnapshotWire): ChunkSnapshot {
    return {
      cx: data.cx,
      cy: data.cy,
      base: data.base || [],
  ...
  async function action(actionType: ActionType, patch: Partial<{ x: number; y: number }>): Promise<void> {
    if (state.busy) {
      return;
    }
    state.busy = true;
  ...
  function actorChunkAndIndex(): ChunkPosition {
  ...
  function drawChunk(chunk: ChunkSnapshot, tile: number, ox: number, oy: number): void {
  ...
  function cellColor(chunk: ChunkSnapshot, index: number, wx: number, wy: number): string {
  ...
  function biomeColors(biome: number): string[] {
  ...
  function hasWaterNeighbor(wx: number, wy: number): boolean {
  ...
  function waterKindAt(wx: number, wy: number): number {
  ...
  function drawGrid(tile: number, ox: number, oy: number): void {
    if (tile < 7) {
      return;
    }
  ...
  function drawActor(tile: number, ox: number, oy: number): void {
  ...
  function startStream(): void {
    connectStream().catch((err: unknown) => {
      if (!isAbortError(err)) {
  ...
  window.addEventListener("keydown", (event: KeyboardEvent) => {
    ...
    if (event.key === " " || event.key.toLowerCase() === "e") void action("harvest", {});
  });
